<template name="protocol-popup">
	<view class="jw-authorize" @touchmove.stop.prevent="clear">
		...
		<view :class="['popup_content', isShow ? 'scroll-open' : '', animation ? 'scroll-animation' : '']">
			...
			<view class="sub-describe">
				<slot name="sub-describe">
					<view>
						我已阅读并同意 <text @click="agreementClick('service')">《服务条款》</text>和<text
							@click="agreementClick('privacy')">《隐私协议》</text>
					</view>
				</slot>
			</view>
			<view class="button-box">
				...
				<slot name="button">
					...
					<button v-if="openType === 'getUserInfo'" class="jw-button get-phone-number-button" @click="getuserinfo">
						同意
					</button>
				</slot>
			</view>
